...
for ((i=0; i<NUM_SIMULATIONS; i++)); do
  ...
  # Run the simulation
  gnome-terminal -- roslaunch llm_generation_trajectory static_8obs.launch

  sleep 10
  
  cd /home/andrea/ros_packages_aggiuntivi/src/llm_generation_trajectory/bag_files/static_obstacles/8_obs
  ...
  gnome-terminal -- rosbag record -O static_obstacles_10.bag --duration=50 /gazebo/model_states /gazebo/bounding_boxes /gptGeneratedPath /callDuration

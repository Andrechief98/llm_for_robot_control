...
for m = 1:numModels
    for i = 1:numObs
        ...
        for j = 1:numSimulations
            ...
            %% Check collisions
            % Check collision is performed on all the points between two
            % consequent point in the generated path, obtained by linearly
            % interpolation with timestep 0.1
            timeStep = 0.1;
            obsBoxes = cell2mat(obs_boxes);
            collision = false;
            
            for p = 1:numPoints-1
                posA = generatedPath(p,:);
                posB = generatedPath(p+1,:);
                delta = posB-posA;
                distSeg = norm(delta);

                numSteps = max(1, ceil(distSeg / timeStep));
                robotTheta = atan2(delta(2),delta(1));

                for s = 0:numSteps
                    alpha = s/ numSteps;
                    interPos = posA +alpha*delta;
                    if checkCollision(interPos, [robot_width, robot_height], robotTheta, obsBoxes)
                        collision = true;
                        break;
                    end
                end
        ...
        fprintf('Model %s, Folder %s: # n_{obs} = %d, Success Rate = %.2f, inferenceTime = %.2f, minObsDist = %.2f, PathLength = %.2f, Smoothness = %.2f\n',...
            models{m}, obsFolders{i}, objectsPerFolder(i, m), successRate(i, m), ...
...
yLimits = { [0, 1.2], [0, 100], [0, 1.2], [0, 2], [-0.1, 0.2] };
yTicks  = { [0.2, 0.4, 0.6, 0.8, 1, 1.2], [0, 20, 40, 60, 80, 100], [0, 0.2,0.4,0.6,0.8,1,1.2], [0, 0.5, 1, 1.5, 2], [-0.1,0,0.1,0.2] };
xLimits = [0, 10];
xTicks  = [2, 4, 6, 8];

% Aggregate metrics
yData    = { successRate, inferenceTimeMean, pathLengthRatioMean, minObsDistMean, smoothnessMean };
...
% errorbar is used to plot the metrics without std_dev
errobar = false;
